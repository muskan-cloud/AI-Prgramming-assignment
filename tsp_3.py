...
class GeneticAlgorithmTSP:
    ...
    def two_opt(self, tour):
        n = len(tour)
        best_distance = self.calculate_fitness(tour)
        improved = True
        while improved:
            improved = False
            for i in range(1, n - 2):
                for j in range(i + 1, n):
                    ...
                    new_distance = self.calculate_fitness(new_tour)
    ...
    def genetic_algorithm(self):
        ...
        best_tour = self.two_opt(best_individual)
        best_cost = self.calculate_fitness(best_tour)
